...
var w = 800, h = 600;
svg.append("text")
      .attr("transform", "translate(10,455)")
      .style("font-size", "70%")
      .text("Budget range");
      ...
      .style("fill", "url(#gradient)").attr("transform", "translate(10,458)");

      var x1 = d3.scale.linear().range([300, 0]).domain([colMax, colMin]);  

      var xAxis1 = d3.svg.axis().scale(x1).orient("bottom");

      key.append("g").attr("class", "x axis").attr("transform", "translate(10,488)").style("font-size", "50%")
...
svg.append("text")
      .attr("transform", "translate(400,459)")
      .style("font-size", "70%")
...
svg.append("text")
      .attr("transform", "translate(9,503.5)")
      .style("font-size", "50%")
      .text("65000");
...
function mouseovered(d) 
{
  ...
  fulltext = "<br><br><br><br><br><br><br><br>"+"<b>"+"Movie: "+d[mov]+ "<b>" + "<br><br>"+"<img src= "+ "https://people.ischool.berkeley.edu/~niavivek/info_webpage/Info_viz_project/scatter_plot/posters/poster_"+d.movie_id+ ".jpeg" 
...
function handleClick(event){
  ...
  myawards = document.getElementById("myawards").value;
...
mybudget = document.getElementById("mybudget").value;
mybudgetgrt = document.getElementById("mybudgetgrt").value;
myimdb = document.getElementById("imdb").value; 
myimdblow = document.getElementById("imdblow").value;  
...
console.log("myawards", myawards);
    console.log(movieterm, actorterm, directorterm, myawards, myimdb);
    draw(movieterm, actorterm, directorterm, mybudget,mybudgetgrt, myawards, myawardsgrt, myimdb,myimdblow, mycriticgrt,  mycriticlow);
return false;
}
...
//                && (parseFloat(d.imdb_score) >= parseFloat(myimdbl)) 
//               )
...
//          ) && (  ( parseInt(d.budget) >= parseInt(mybudget))
//                && (parseInt(d.Winner) >= parseInt(myawards))
//                && (parseFloat(d.imdb_score) >= parseFloat(imdb)) 
//               )
...
//     };
  ...
  var disp = function(d) { 
    ...
    right = (  ( parseInt(d.budget) >= parseInt(mybudget))
...
init= ( (movieterm == "        ") && (actorterm == "        ")&& (directorterm == "        "))
  ...
  if (left && right)
        ...
        console.log("fail","imdb", left, right,  (parseInt(d.Winner) <= parseInt(myawardsgrt))  
         , (parseInt(d.Winner) >= parseInt(myawards))
                ,(parseInt(d.Winner) <= parseInt(myawardsgrt))
