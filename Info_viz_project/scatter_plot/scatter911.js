...
var color = d3.scale.linear().range(["#ff6464", "#000000"]);
...
svg.append("text")
      ...
      legend.append("stop").attr("offset", "100%").attr("stop-color", "#000000").attr("stop-opacity", 1);
...
myimdb = document.getElementById("myimdb").value;
mycritic = document.getElementById("mycritic").value;
console.log("myawards", myawards);
    console.log(movieterm, actorterm, directorterm, myawards, myimdb);
    draw(movieterm, actorterm, directorterm, mybudget, myawards, myimdb, mycritic );
return false;
}
...

{
  ...
  var disp = function(d) { 
    ...
    right = (( parseInt(d.budget) >= parseInt(mybudget))
               && (parseInt(d.Winner) >= parseInt(myawards))
               && (parseFloat(d.imdb_score) >= parseFloat(myimdb)) 
               && (parseFloat(d.critic_score) >= parseFloat(mycritic)) 
              )
